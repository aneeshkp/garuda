...
int add_IP_Address(char * IP, struct rtnl_handle * rth, struct my_interface * iface,int action)
{
    ...
    req.ifa.ifa_index = iface->index ; // get the loopback index
    req.ifa.ifa_scope = 0 ;
...
void  get_interface_index(struct my_interface *iface){
    ...
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr->sa_family==AF_INET && (strcmp(ifa->ifa_name,iface->name)==0)) {
            interface_found=1;
            sa = (struct sockaddr_in *) ifa->ifa_addr;
            iface->index=if_nametoindex(ifa->ifa_name);
            //iface->ipaddr = inet_ntoa(sa->sin_addr);
            //if ((ifa->ifa_flags & IFF_UP) == 0){
            iface->status=(ifa->ifa_flags & IFF_UP);
            iface->index=if_nametoindex(ifa->ifa_name);

            if(strcmp(inet_ntoa(sa->sin_addr),iface->ipaddr)==0){
              iface->vip_status=1;
              break;
             }
...
{
     interface_event_action("192.168.1.100","eno2");
